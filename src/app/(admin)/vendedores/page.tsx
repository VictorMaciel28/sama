...
export default function VendedoresPage() {
  ...
  const [formIdExterno, setFormIdExterno] = useState('')
  ...
  const openNewUser = () => {
    ...
    setFormTelefone('')
    setFormTipo('')
    setFormNivel('ADMINISTRADOR')
    setFormIdExterno('')
    setFormRazaoSocial('')
    setFormEnderecoRazao('')
    setFormNomeRepresentante('')
    setFormEnderecoRepresentante('')
    setFormCpf('')
    setFormIdentidade('')
  ...
  const openEdit = (v: Vendedor) => {
    ...
    setFormIdExterno(String(v.id_vendedor_externo || '').trim())
    setShowModal(true)
  }

  const onSave = async () => {
    if (isCreating) {
      const ext = formIdExterno.trim()
      if (!formNome.trim() || !formEmail.trim() || !formPassword.trim()) {
      ...
      const payload: Record<string, unknown> = {
        ...
        tipo_acesso: formTipo ? formTipo : null,
        nivel_acesso: formNivel ? formNivel : null,
      }
      const res = await fetch('/api/vendedores', {
        method: 'POST',
      ...
      const json = await res.json()
      if (!json?.ok) {
        alert(json?.error ?? 'Falha ao criar usuário')
        return
      }
      setShowModal(false)
      setIsCreating(false)
    ...
    <div className="p-3">
        ...
        <div className="card-body">
          <div className="d-flex align-items-center justify-content-between mb-3">
            ...
            <div className="d-flex align-items-center gap-2 flex-wrap justify-content-end">
              <button type="button" className="btn btn-outline-primary" onClick={openNewUser}>
                Incluir usuário
              </button>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>{isCreating ? 'Incluir usuário' : 'Editar vendedor'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            <Row className="g-3">
              <Col md={4}>
                ...
                <Form.Control
                  type="text"
                  value={formIdExterno}
                  onChange={(e) => isCreating && setFormIdExterno(e.target.value)}
                  readOnly={!isCreating}
              ...
              <Col md={12}>
                <Form.Label>{isCreating ? 'Senha (obrigatória no cadastro)' : 'Senha (preencher para permitir login)'}</Form.Label>
                <div className="input-group">
                  <Form.Control
                    type={showPassword ? 'text' : 'password'}
                    value={formPassword}
                    onChange={(e) => setFormPassword(e.target.value)}
                    placeholder={isCreating ? 'Senha inicial' : 'Deixe em branco para não alterar'}
                  />
                  <button
                    type="button"
