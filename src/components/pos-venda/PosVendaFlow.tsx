...
export type PosVendaVariant = 'public' | 'admin'
...
export default function PosVendaFlow({ variant }: { variant: PosVendaVariant }) {
  const isPublic = variant === 'public'
  const idSuffix = variant === 'public' ? '' : '-admin'

  const [numeroNota, setNumeroNota] = useState('')
  const [cnpj, setCnpj] = useState('')
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const [pesquisaAlertVariant, setPesquisaAlertVariant] = useState<'danger' | 'warning'>('danger')
  ...
  const handlePesquisar = async (e: FormEvent) => {
    e.preventDefault()
    setError('')
    setPesquisaAlertVariant('danger')
    setResumo(null)
    setDetalhe(null)
    setMarcados({})
    setRevisaoError('')
    ...
    try {
      const res = await fetch('/api/public/revisar-pedido/pesquisar', {
        ...
        body: JSON.stringify({ numero: n, ...(c ? { cnpj: c } : {}) }),
      })
      const json = await res.json().catch(() => null)
      if (!res.ok || !json?.ok) {
        setPesquisaAlertVariant(res.status === 409 ? 'warning' : 'danger')
      ...
      <div className="col-12">
        <div className="card border-0 shadow-sm mb-4">
          <div className="card-body p-4">
            {isPublic ? (
              <>
                ...
                <p className="text-muted mb-4">
                  Informe apenas o número da nota fiscal. Se existir mais de uma nota com o mesmo número, o sistema
                  pedirá também o CNPJ do destinatário.
                </p>
              </>
            ) : (
              <>
                <h5 className="mb-2 fw-semibold">Consulta da nota fiscal</h5>
                <p className="text-muted small mb-4">
                  Busca pelo número da NF (CNPJ opcional se houver mais de uma nota com o mesmo número). Em seguida
                  revise os itens e registre a
                  solicitação com imagens.
                </p>
              </>
            )}
            <form onSubmit={handlePesquisar}>
              <div className="row g-3 align-items-end">
                <div className="col-md-8 col-lg-7">
                  <label className="form-label fw-semibold">Número da nota fiscal</label>
                  <input
              ...
              <div className="row g-3 mt-1">
                <div className="col-12">
                  <label className="form-label text-muted small mb-1">CNPJ do destinatário (opcional)</label>
                  <input
                    className="form-control"
                    value={cnpj}
            ...
            {error && (
              <div className={`alert mt-3 mb-0 ${pesquisaAlertVariant === 'warning' ? 'alert-warning' : 'alert-danger'}`}>
                {error}
              </div>
            )}
          </div>
          ...
          <div className="card border-0 shadow-sm mb-4">
            <div className="card-body p-4">
              ...
              <form onSubmit={handleSolicitar} className="border-top pt-4">
                <div className="mb-3">
                  <label className="form-label" htmlFor={`pos-venda-observacoes${idSuffix}`}>
                    Observações
                  </label>
                  <textarea
                    id={`pos-venda-observacoes${idSuffix}`}
                    className="form-control"
                    rows={4}
                    value={observacoes}
                  ...
                  <div className="col-md-3">
                    ...
                    <div className="form-check mt-2">
                      <input
                        ...
                        type="checkbox"
                        id={`pos-venda-telefone-whatsapp${idSuffix}`}
                        checked={telefoneEhWhatsapp}
                        onChange={(e) => setTelefoneEhWhatsapp(e.target.checked)}
                      />
                      <label className="form-check-label" htmlFor={`pos-venda-telefone-whatsapp${idSuffix}`}>
                        Sim
                      </label>
                    </div>
                  </div>
